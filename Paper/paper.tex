...
Complementing \cite{Applegate2009}, this research intemegrates the concept of Embedded Finance and the Ecosystem Driver model described by \cite{Weill2018}. In this model, banks no longer act as standalone destinations but embed their financial services (payments, lending, savings) directly into the customer’s daily digital activities (e.g., ride-hailing, e-commerce).  
...
\subsection{Legacy state of Bank Artos}
...
The most damning metric for Bank Artos was its Operating Expenses to Operating Income (BOPO) ratio. A BOPO ratio above 100\% indicates that a bank spends more to operate than it earns.
...
\subsection{Comparative Industry Analysis}

To fully understand Bank Jago’s position, it must be benchmarked against its  competitors in the Indonesian digital banking market.

\subsubsection{SeaBank}

SeaBank, owned by Sea Group (Shopee), is Jago’s fiercest competitor. Like Jago, it was born from the acquisition of a small bank (Bank BKE).

\begin{enumerate}
	\item Strategy: SeaBank employs a "High-Yield" strategy, offering aggressive interest rates on daily deposits (often topping the market charts) to attract users rapidly. It leverages the massive transaction volume of Shopee for payments and loans.  
	
	\item Performance: SeaBank achieved profitability faster than Jago, recording a net profit of IDR 379 billion in 2024. However, its NPL ratios have historically been higher (ranging from 1.7\% to 2.0\%) compared to Jago’s 0.2\%.  
	
	\item Contrast: Jago focuses on "sustainable low-cost funds" (CASA) through transactional utility (Gojek integration), whereas SeaBank relies more on "expensive funds" attracted by high interest rates. Jago’s strategy is arguably more sustainable in a low-interest-rate environment, while SeaBank’s is more sensitive to rate changes.
\end{enumerate}

\subsubsection{Jenius}

Jenius was the first digital bank in Indonesia, launching in 2016 within BTPN.

\begin{enumerate}
	\item Strategy: Jenius launched as a standalone lifestyle banking app. It created the category and educated the market.
	
	\item Performance: Despite its head start, Jenius had ~5.9 million users in 2024 , significantly fewer than Jago’s 15.3 million.  
	
	\item Contrast: Jenius’s slower growth highlights the power of the "Ecosystem Driver" model. Jago grew faster by being inside Gojek than Jenius did by marketing a standalone app. Additionally, Jenius has struggled with technical stability and high NPLs in its "Flexi Cash" product compared to Jago’s prudent partnership lending.   
\end{enumerate}
